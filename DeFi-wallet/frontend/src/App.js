...
function App() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const balanceEth = await wallet.callStatic.getUnderlyingEthBalance(); //callStatic() is etheres equivalent of .call(), can be used instead of .send()
      const balanceDai = await wallet.getUnderlyingBalance(addresses.dai);
      //These are the same as setState() in class components:
      setSigner(signer);
      setWallet(wallet);
      setDai(dai);
