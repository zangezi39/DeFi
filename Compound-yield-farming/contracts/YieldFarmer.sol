...
contract YieldFarmer is ICallee, DydxFlashloanBase, Compound {
  ...
  function openPosition(
      address _solo,	//dYdX contract
      address _token,   //token to borrow
      address _cToken,  //associated cToken
    ...
    _initiateFlashloan(_solo, _token, _cToken, Direction.Deposit, _amountProvided - 2, _amountBorrowed);  //-2 wei flashloan fee
  }
  ...
  function closePosition(
    ...
    require(msg.sender == owner, 'only owner');
    IERC20(_token).transferFrom(msg.sender, address(this), 2); //2 wei flashloan fee
    claimComp();
  ...
  function _initiateFlashloan(
    address _solo,        //dYdX
    address _token,       //token being borrowed
    address _cToken,      //associated Compound cToken
    Direction _direction, //Deposit or Withdraw
    uint _amountProvided, //amount of token as collateral
    uint _amountBorrowed  //borrow amount being requested
  )
    internal
  {
    ISoloMargin solo = ISoloMargin(_solo);  //pointer to dYdX smart contract
    ...
    solo.operate(accountInfos, operations); //initiates the flashloan
  }
}
